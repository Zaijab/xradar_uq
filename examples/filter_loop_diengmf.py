...
class DIEnGMF(AbstractFilter, strict=True):
...
class ZConvexHull(eqx.Module, ptx.ConvexHull):
    def ConvexHull__post_init__(self):
        print("AAAAAAAAAAAAAAAAAAAAAA")
        pass


@eqx.filter_jit
def reachable_set_no_thrust(thrust_key, ensemble, time_horrizon, delta_v_magnitude, num_simulations, dynamical_system, n_directions=100):
    # thrust_ensemble = simulate_thrust(thrust_key, ensemble, num_simulations, delta_v_magnitude)
    thrust_ensemble = eqx.filter_vmap(dynamical_system.flow)(0.0, time_horrizon, ensemble)
...
def rejection_sample(
    ...
    _, final_sample, _ = eqx.internal.while_loop(
        cond_fun, body_fun, carry_init, kind="checkpointed", max_steps=10 ** 4
    )
    return final_sample
...
def rejection_sample(
    ...
    mean: Float[Array, "state_dim"], 
    cov: Float[Array, "state_dim state_dim"],
    hull_vertices: Float[Array, "n_vertices state_dim"],
) -> Float[Array, "state_dim"]:
    ...
    def body_fun(carry):
        ...
        rng, subkey = jax.random.split(rng)
        candidate = jax.random.multivariate_normal(subkey, mean, cov)
        # Use simpler containment test for robustness
        pass_discriminator = point_in_convex_hull_simple(candidate, hull_vertices)
        sample = jnp.where(pass_discriminator, candidate, sample)
        accepted = accepted | pass_discriminator
        return (rng, sample, accepted)
    ...
    carry_init = (key, jnp.zeros_like(mean), False)
    _, final_sample, _ = eqx.internal.while_loop(
        cond_fun, body_fun, carry_init, kind="bounded", max_steps=1
    )
    return final_sample
...
stochastic_filter = EnGMF(sampling_function=jax.tree_util.Partial(rejection_sample, hull_vertices=current_reachable_set.vertices))
